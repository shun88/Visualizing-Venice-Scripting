...
key npcKey = "99f23f32-c31b-4802-bd2e-aa4b8f0f7a68";
...
default {
    state_entry() {
        llSay(0, "puppeteer starts running");
...
state spawn
{
    state_entry() {
        ...
        llSetObjectDesc(wasKeyValueSet("key", osNpcCreate(llList2String(name, 0), llList2String(name, 1), llGetPos() + <1, 0, 0>, "appearance"), llGetObjectDesc()));
        ...
        llSetTimerEvent(10);
        //osNpcLoadAppearance(wasKeyValueGet("key", llGetObjectDesc()), "maleAppearance");
    }
    timer() {
        llSetTimerEvent(0);
